...
class db_manage {
	...
	/**
	 * 数据库备份
	 * 参数：备份哪个表(可选),备份目录(可选，默认为backup),分卷大小(可选,默认1000，即1M)
	 *
	 * @param $string $dir        	
	 * @param int $size        	
	...
	function backup($tablename = '', $dir, $size) {
		...
		$date = date('y-m-d_H-i');
		$dir = $dir ? $dir : "./sundry/static_file/sql/{$date}/";
		$size = $size ? $size : 1000;
		// 只备份某个表
		if (! empty ( $tablename )) {
	...
	private function _retrieve() {
		...
		$value .= '-- Created by DBManage class, By 郝海川博客 ' . $this->ds;
		$value .= '-- http://ithhc.cn ' . $this->ds;
		$value .= '--' . $this->ds;
	...
	private function _write_file($sql, $filename, $dir) {
		...
		if (! file_exists ( $dir )) {
			mkdir ( $dir );
			file_put_contents($dir,'ithhc.cn');
		}
		$re = true;
