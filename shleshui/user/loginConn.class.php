<?php
	...
	class loginConn extends Conn{
		...
		public function login(){
			header('location:http://yakelvxing.com/index.php?m=user&a=user_login');
			exit;
			$this -> output();
		}
		...
		public function register(){
			header('location:http://yakelvxing.com/index.php?m=user&a=user_reg');
			exit;
			$type = C('user_rep');
			if($type=='1'){
				//需要购买验证码
		...
		public function run(){
			$_COOKIE['last_url'] = isset($_COOKIE['last_url']) ? $_COOKIE['last_url'] : '?';
		...
		public function fankui(){
			...
			$pdo = get_pdo();
			$uid = empty($_SESSION['user']['user_id']) ? '0' : $_SESSION['user']['user_id'];
			$title = isset($_POST['title']) ? $_POST['title'] : '无标题';
			$content = isset($_POST['content']) ? $_POST['content'] : '无内容';
			//echo $uid.'<br/>'.$title.'<br/>'.$content;
			if(!empty($uid)){
				$res = $pdo -> query('select user_name from '.DB_PRE."user where user_id='{$uid}' limit 1",'');
				$user_name = empty($res) ? '游客' : $res['user_name'];
			}else{
				$user_name = '游客';
			}
			$pdo -> sql('insert into '.DB_PRE."lyfk(fasongzhe,neirong,title)values('{$user_name}','{$content}','{$title}')");
