...
function register()
{
	...
	if($flag)
	{
		...
		$link_href = url(array('entrance'=>$global['entrance'],'channel'=>'user'));
		$link_text = $lang['enter_user_center'];
	}else{
		$link_href = url(array('entrance'=>$global['entrance'],'channel'=>'user','mod'=>'register'));
...
function user_login()
{
	...
	$smarty->assign('info_text',$info_text);
	$smarty->assign('link_text',$link_text);
	$smarty->assign('link_href',url(array('entrance'=>$global['entrance'],'channel'=>'user')));
}
...
function add_comment()
{
	...
	if($channel == '' || $com_page_id == '' || $com_rank == '' || $com_email == '' || $com_text == '')
	{
		...
		if(intval(get_varia('sentmail')))
		{
			...
			call_send_email($email_title,$email_text,$global['user_id'],$com_email);
		}
...
function add_research()
{
	...
	if(count($list) > 0)
	{
		...
		$obj = new research();
		$obj->set_value('res_user_id',$global['user_id']);
