				...
				<div class="unit" name="roll_unit">
					<div class="img">
						<table>
							<tr>
								<td>
									<a href="{url channel='goods' id=$item.goo_id}" target="_blank"><img src="{$S_ROOT}{$item.goo_x_img}" alt="{$item.goo_title}" onload="picresize(this,150,150)" /></a>
								</td>
							</tr>
						</table>
					</div>
					<div class="title">
						<a href="{url channel='goods' id=$item.goo_id}" title="{$item.goo_title}" target="_blank">{$item.short_title}</a>
					</div>
				</div>
				{/foreach}
			</div>
