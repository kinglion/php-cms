...
<div class="here">
	...
	<div class="link">
		...
		{if $channel_title}<a href="{url prefix=$S_ROOT|cat:$global.entrance|cat:'?/search' key=$global.key}">{$channel_title}</a>{/if}
		{if $page_title}<a href="{url prefix=$S_ROOT|cat:$global.entrance|cat:'?/search' key=$global.key}">{$page_title|truncate:20}</a>{/if}
		<div class="clear"></div>
	</div>
	<div class="right"></div>
</div>
...
{$prefix = $S_ROOT|cat:$global.entrance|cat:'?/search/key-'|cat:$global.key}
